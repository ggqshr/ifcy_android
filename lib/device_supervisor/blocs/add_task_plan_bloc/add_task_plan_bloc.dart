...
class AddTaskPlanBloc extends Bloc<AddTaskPlanEvent, AddTaskPlanState> {
  ...
  AddTaskPlanBloc(this.userLoginRepositories, [repo])
  ...
  Stream<AddTaskPlanState> mapEventToState(
    ...
    if (event is FetchData) {
      try {
        ...
        yield AfterFetchedState.fetchSuccess(
            systems, cycles, persons, userLoginRepositories.currentBuild);
      } catch (e) {
    ...
    if(event is SubmitData){
      ...
      try{
        bool isSuccess= await addTaskPlanRepositories.submitting(event.taskPlanEntity);
        if(isSuccess){
